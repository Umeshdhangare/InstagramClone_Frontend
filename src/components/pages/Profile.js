...
function Profile(props) {
	...
	const followHandler = async () => {
		try {
			if (followed) {
				await axiosJWT.put(
					config.url + `user/${username}/unfollow`,
					{},
					{
				...
				await axiosJWT.put(
					config.url + `user/${username}/follow`,
					{},
					{
						headers: { Authorization: "Bearer " + currentUser.accessToken },
					}
	...
	useEffect(() => {
		const fetchUser = async () => {
			const res = await axios.get(config.url + `user/u/${username}`);
			setCurrentUser(res.data.user);
			const pst = await axios.get(config.url + `post/u/${username}`);
			setPosts(
				pst.data.sort((p1, p2) => {
				...
				<div className="profileWrapper">
					...
					<div className="profileData">
						<div className="profileSettings">
							<span className="profileSettingsUsername">
